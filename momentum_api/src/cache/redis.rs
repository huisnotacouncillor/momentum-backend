use momentum_core::db::{DbPool, models::auth::User};
use diesel::prelude::*;
...
pub async fn get_user_current_workspace_id_cached(
    client: &redis::Client,
    pool: &DbPool,
    user_id: Uuid,
) -> Option<Uuid> {
    ...
    let user = match momentum_core::schema::users::table
        .filter(momentum_core::schema::users::id.eq(user_id))
