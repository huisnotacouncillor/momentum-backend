...
impl WebSocketCommandHandler {
    pub fn new(db: Arc<DbPool>, asset_helper: Arc<momentum_core::utils::AssetUrlHelper>) -> Self {
        Self {
            db,
    ...
    pub fn get_asset_helper(&self) -> Arc<momentum_core::utils::AssetUrlHelper> {
    ...
    async fn handle_create_team(
        ...
        let result =
            conn.transaction::<momentum_core::db::models::team::Team, diesel::result::Error, _>(|conn| {
                let new_team = momentum_core::db::models::team::NewTeam {
                    name: req.name.clone(),
                ...
                let team: momentum_core::db::models::team::Team = diesel::insert_into(schema::teams::table)
                    .values(&new_team)
                    .get_result::<momentum_core::db::models::team::Team>(conn)?;
                let new_team_member = momentum_core::db::models::team::NewTeamMember {
                    user_id,
                    team_id: team.id,
    ...
    async fn handle_update_team(
        ...
        let existing_team = match schema::teams::table
            ...
            .select(momentum_core::db::models::team::Team::as_select())
            .first::<momentum_core::db::models::team::Team>(&mut conn)
        {
            Ok(team) => team,
        ...
        let updated = diesel::update(schema::teams::table.filter(schema::teams::id.eq(team_id)))
            ...
            .get_result::<momentum_core::db::models::team::Team>(&mut conn);
        match updated {
            Ok(team) => Ok(serde_json::to_value(team).unwrap()),
            Err(e) => {
                if e.to_string().contains("team_key") {
    ...
    async fn handle_query_teams(&self, ctx: RequestContext) -> Result<serde_json::Value, AppError> {
        ...
        let list = match schema::teams::table
            ...
            .load::<momentum_core::db::models::team::Team>(&mut conn)
        {
            Ok(list) => list,
    ...
    async fn handle_add_team_member(
        ...
        momentum_core::services::team_members_service::TeamMembersService::add(
            &mut conn,
            &ctx,
            team_id,
            data.user_id,
    ...
    async fn handle_update_team_member(
        ...
        momentum_core::services::team_members_service::TeamMembersService::update(
            &mut conn,
            &ctx,
            team_id,
            member_user_id,
    ...
    async fn handle_remove_team_member(
        ...
        let mut conn = self
            ...
            .get()
            .map_err(|_| AppError::Internal("Database connection failed".to_string()))?;
        momentum_core::services::team_members_service::TeamMembersService::remove(
            &mut conn,
            &ctx,
            team_id,
            member_user_id,
    ...
    async fn handle_list_team_members(
        ...
        let mut conn = self
            ...
            .get()
            .map_err(|_| AppError::Internal("Database connection failed".to_string()))?;
        let members = momentum_core::services::team_members_service::TeamMembersService::list(
