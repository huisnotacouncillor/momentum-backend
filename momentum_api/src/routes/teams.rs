...
use momentum_core::services::{team_members_service::TeamMembersService, teams_service::TeamsService};
use axum::{
    Json,
    extract::{Path, State},
    http::StatusCode,
...
pub use momentum_core::services::teams::types::{CreateTeamRequest, UpdateTeamRequest};
...
pub async fn create_team(
    ...
    match TeamsService::create(&mut conn, &ctx, &req) {
        ...
        Err(e) => match e {
            momentum_core::error::AppError::Validation { message } => {
...
pub async fn update_team(
    ...
    match TeamsService::update(&mut conn, &ctx, team_id, &req) {
        ...
        Err(e) => match e {
            ...
            momentum_core::error::AppError::NotFound { .. } => {
                let response = ApiResponse::<()>::not_found("Team not found");
                (StatusCode::NOT_FOUND, Json(response)).into_response()
            }
            _ => {
...
pub async fn delete_team(
    ...
    match TeamsService::delete(&mut conn, &ctx, team_id) {
        ...
        Err(momentum_core::error::AppError::NotFound { .. }) => {
            let response = ApiResponse::<()>::not_found("Team not found");
            (StatusCode::NOT_FOUND, Json(response)).into_response()
        }
        Err(_) => {
...
pub async fn add_team_member(
    ...
    match TeamMembersService::add(&mut conn, &ctx, team_id, payload.user_id, role_str) {
        ...
        Err(momentum_core::error::AppError::Validation { message }) => {
...
pub async fn get_team_members_list(
    ...
    let members = members
        ...
        .map(|(member, user)| TeamMemberInfo {
            user: momentum_core::db::models::auth::UserBasicInfo {
                id: user.id,
                name: user.name,
...
pub async fn get_user_teams(
    ...
    use momentum_core::schema::team_members::dsl::*;
    use momentum_core::schema::teams::dsl as team_dsl;
