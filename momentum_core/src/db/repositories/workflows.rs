...
use crate::db::models::workflow::{NewWorkflow, NewWorkflowState, Workflow, WorkflowState};
...
impl WorkflowsRepo {
    pub fn list_by_team(
        ...
        team: uuid::Uuid,
    ) -> Result<Vec<Workflow>, diesel::result::Error> {
    ...
    pub fn list_states(
        conn: &mut PgConnection,
        wf: uuid::Uuid,
    ) -> Result<Vec<WorkflowState>, diesel::result::Error> {
        use crate::schema::workflow_states::dsl::*;
        workflow_states
            .filter(workflow_id.eq(wf))
            .order(position.asc())
    ...
    pub fn list_states_by_workflow(
        conn: &mut PgConnection,
        wf: uuid::Uuid,
    ) -> Result<Vec<WorkflowState>, diesel::result::Error> {
        use crate::schema::workflow_states::dsl::*;
    ...
    pub fn list_team_default_states(
        conn: &mut PgConnection,
        team: uuid::Uuid,
    ) -> Result<Vec<WorkflowState>, diesel::result::Error> {
    ...
    pub fn insert_team_default_state(
        conn: &mut PgConnection,
        _team_id: uuid::Uuid,
        new_state: &NewWorkflowState,
    ) -> Result<WorkflowState, diesel::result::Error> {
    ...
    pub fn find_team_default_state_by_id(
        conn: &mut PgConnection,
        team: uuid::Uuid,
        state_id: uuid::Uuid,
    ) -> Result<Option<WorkflowState>, diesel::result::Error> {
        use crate::schema::{workflow_states, workflows};
        workflow_states::table
            .inner_join(workflows::table.on(workflow_states::workflow_id.eq(workflows::id)))
            .filter(workflows::team_id.eq(team))
