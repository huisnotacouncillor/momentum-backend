...
fn validate_register_and_login_inputs() {
    use rust_backend::validation::auth::{
        validate_register_request, validate_login_request, UpdateProfileChanges,
        validate_update_profile,
    };

    // register
    assert!(validate_register_request("John Doe", "john_doe", "john@example.com", "StrongP4ss!").is_ok());
    ...
    // profile update
    let ok_changes = UpdateProfileChanges { name: Some("John"), username: None, email: None, avatar_url: None };
    assert!(validate_update_profile(&ok_changes).is_ok());
    let empty_changes = UpdateProfileChanges { name: None, username: None, email: None, avatar_url: None };
